...
let adlist = require('../models/advertisement');
...
exports.isAllowed = async function (req, res, next) {
    ...
    try {
        ...
        let adlistItem = await adlist.findById(id).populate('owner');

        // If there is no item found.
        if (adlistItem == null) {
            throw new Error('Item not found.') // Express will catch this on its own.
        }
        else if (adlistItem.owner != null) { // If the item found has a owner.

            if (adlistItem.owner._id != req.payload.id) { // If the owner differs.
